...
const getProject = (filename) => {
  try {
    const rawProjectData = fs.readFileSync(path.resolve("./public", `projects/${filename}`), "utf-8")
...
const getProjects = () => {
  try {
    return fs.readdirSync(path.resolve("./public", "projects")).reduce(
      (projects, filename) => {
...
const getInfo = (slug) => {
  try {
    const rawInfoData = fs.readFileSync(path.resolve("./public", `info/${slug}`), "utf-8")
...
const getInfoList = () => {
  try {
    return fs.readdirSync(path.resolve("./public", "info")).reduce(
      (infoList, filename) => {
        const slug = path.parse(filename).name
        if(!slug.startsWith(".")){
